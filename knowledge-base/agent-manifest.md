...
1. **JSON-only** — `squad.json` + `CLAUDE.md`. Defines tabs, prompt, colors, icon. Uses built-in tab components.
2. **Custom React** — `squad.json` + `bundle.js`. Custom components. Import `@squad/*` as peer deps.
...
- **Built-in:** `app/src/agents/builtin/` — `personalAssistantAgent`
  ...
  The richer catalog lives in Squad Store under `store/agents/*`.
- **Installed:** `~/.squad/agents/{id}/squad.json` — installed from bundled Squad Store or downloaded from GitHub.
- **Override rule:** installed definition with same id as builtin → overrides builtin (dedup in `app/src/stores/agent-configs.ts`).

## Store install flow

Squad-owned Store agents are release-bundled:
...
`POST /v1/store/installs` with `repo: "squad-store/<agent-id>"`
copies the package to `~/.squad/agents/<agent-id>/` and writes
`.source.json` with `source: "squad-store"`, `version`, and
...
Store manifests must also not seed `.squad/activity.json` or
`.squad/activity/activity.json`; fresh Store agents start with an empty
...
After a bundled package update, Squad copies newly-added packaged
...
`owner/repo` URL and Squad downloads `squad.json`, `CLAUDE.md`,
`icon.png`, `bundle.js` → `~/.squad/agents/{id}/`. The desktop
New Agent modal is Store-first for quick installs; developers can still
import their own repos from the same modal.
...
First-run onboarding is a minimal guided setup aimed at developers who
want to feel a real agent working in the first few minutes, not a static
tour or dense HUD:

1. Welcome screen shows the Squad logo and two cards: start tutorial or skip.
...
6. Squad writes one Skill and one Routine, then starts real work when apps
   are ready.
...
**Import:** "New Workspace > Import from GitHub". Paste `owner/repo`. Squad downloads workspace.json, installs all agent defs, creates workspace, creates agent instances w/ CLAUDE.md + seed files. All agents chat-ready immediately.

Engine route: `POST /v1/store/workspaces/install-from-github`. Rust impl: `squad_engine_core::store::install_workspace_from_github`. Server wiring: `engine/squad-engine-server/src/routes/store.rs`.
...
## Workspace
- Storage: `~/.squad/workspaces/workspaces.json` (index) + one dir per workspace `~/.squad/workspaces/{Name}/`. `SQUAD_DOCS` env var overrides the root.
- First launch: welcome screen, create first workspace
- Engine routes: `GET /v1/workspaces`, `POST /v1/workspaces`, `POST /v1/workspaces/:id/rename`, `DELETE /v1/workspaces/:id`, `PATCH /v1/workspaces/:id/provider` (`engine/squad-engine-server/src/routes/workspaces.rs`). Frontend reaches them via `@squad/engine-client` — no Tauri commands in the path.
...
Lives in `app/src-tauri/src/squad_prompt/` for the Squad desktop app. Covers the app-context dictionary, concise user voice, the silent interaction loop (classify request, check info, check integrations, decide approval, execute, consider memory), Skills/memory guidance, Routines guidance, and Composio guidance. Passed to the engine at boot via env vars `SQUAD_APP_SYSTEM_PROMPT` + `SQUAD_APP_ONBOARDING_PROMPT` — the engine keeps them as opaque strings. Callers can also override per-session via the `systemPrompt` field on `startSession`.

**Agent-context layer (engine-owned).**
Built in `engine/squad-engine-core/src/agents/prompt.rs::build_agent_context`:
1. **Working directory block** — hard rules scoping file I/O to `<agent-root>`.
2. Mode file `.squad/prompts/modes/<mode>.md` (optional, user-editable).
3. Learnings snapshot — `.squad/learnings/learnings.json`, text fields only, rendered as bounded background context. IDs/timestamps stay storage/UI-only.
4. Skills index — `.agents/skills/` via `squad_skills::build_skills_index`.
5. Integrations block — based on `.squad/integrations.json` if present.

`CLAUDE.md` is read by the CLI (claude/codex) itself at startup, not injected by the engine.

Users cannot edit the product prompt — it's compiled into the app binary. Per-agent surfaces that ARE user-editable: `CLAUDE.md` (job description), `.agents/skills/` (skills), `.squad/learnings/learnings.json` (learnings), `.squad/prompts/modes/*.md` (mode overrides).

## Board / Activity tab
`@squad/board::AIBoard` = `KanbanBoard` + `KanbanDetailPanel` + `ChatPanel`. Each card = activity from `.squad/activity/activity.json`. Click → opens chat w/ conversation history. App `board-tab.tsx` ~140 lines, thin store wrapper.
