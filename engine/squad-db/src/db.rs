...
///
/// - Release builds: `~/.squad/` — what real users see.
...
///
/// Override via `SQUAD_HOME` env var (wins regardless of build profile).
/// Delete this directory for a completely clean slate.
pub fn squad_dir() -> PathBuf {
    if let Ok(override_path) = std::env::var("SQUAD_HOME") {
