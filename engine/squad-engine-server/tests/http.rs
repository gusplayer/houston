...
async fn health_authorized_with_bearer() {
    ...
    assert!(res.headers().get("x-squad-engine-version").is_some());
...
mod ws_helpers {
    ...
    use squad_engine_protocol::{ClientRequest, EngineEnvelope, EnvelopeKind};
    ...
    pub async fn spawn_server() -> (SocketAddr, String, squad_ui_events::BroadcastEventSink) {
...
async fn ws_no_subscription_receives_nothing() {
    ...
    events.emit(SquadEvent::ComposioCliReady);
    events.emit(SquadEvent::ActivityChanged {
