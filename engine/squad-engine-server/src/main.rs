//! `squad-engine` binary entry point.
//!
...
async fn main() {
    ...
    // the main thread until finished and delayed the `SQUAD_ENGINE_LISTENING`
    ...
    let path_init = tokio::task::spawn_blocking(|| {
        squad_terminal_manager::claude_path::init();
    });
    ...
    // `squad_engine_core::git_bash::ensure_bundled_bash`, so a
    ...
    tokio::task::spawn_blocking(|| {
        if let Some(bash) = squad_engine_core::git_bash::ensure_bundled_bash() {
    ...
    println!(
        "SQUAD_ENGINE_LISTENING port={} token={}",
        actual.port(),
        cfg.token
    );
    tracing::info!(
        "squad-engine {} (protocol v{}) listening on {}",
        ENGINE_VERSION,
        PROTOCOL_VERSION,
        actual
    );
    ...
    let tunnel_identity = match squad_tunnel::ensure(&cfg.home_dir, &cfg.tunnel_url).await {
        Ok(identity) => {
            tracing::info!(
                target: "squad_tunnel",
    ...
    // `SquadEvent`s for the frontend to react to and never block the
...
fn spawn_cli_lifecycles(state: Arc<ServerState>) {
    {
        ...
        tokio::spawn(async move {
            squad_composio::lifecycle::ensure_and_upgrade(sink, db).await;
        });
    }
    {
        let sink = state.engine.events.clone();
        let db = state.engine.db.clone();
        tokio::spawn(async move {
            squad_claude_installer::ensure_and_upgrade(sink, db).await;
        });
    }
}
