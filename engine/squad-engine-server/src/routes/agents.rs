//! `/v1/agents/*` REST routes — typed CRUD over `.squad/<type>/<type>.json`.
//!
...
fn emit(state: &ServerState, event: SquadEvent) {
...
async fn create_activity(
    ...
    let root = resolve_root(&q.agent_path)?;
    squad_engine_core::agents::store::ensure_squad_dir(&root)?;
    let result = activity::create(&root, input)?;
    emit(
        &st,
        SquadEvent::ActivityChanged {
...
// — owned by `squad_engine_core::conversations`. Not duplicated here.
