...
async fn cli_installed(State(_st): State<Arc<ServerState>>) -> Json<CliInstalled> {
    Json(CliInstalled {
        installed: squad_claude_installer::is_installed(),
    })
}

async fn status(State(st): State<Arc<ServerState>>) -> Json<ClaudeStatus> {
    let installed = squad_claude_installer::is_installed();
    let install_path = squad_claude_installer::cli_path()
        .to_string_lossy()
        .to_string();

    let pinned_version = squad_cli_bundle::load_bundled_manifest()
    ...
    tokio::spawn(async move {
        ...
        match result {
            Ok(_) => {
                ...
                sink.emit(squad_ui_events::SquadEvent::ClaudeCliReady);
            }
            Err(e) => {
                sink.emit(squad_ui_events::SquadEvent::ClaudeCliFailed { message: e });
            }
        }
    });
