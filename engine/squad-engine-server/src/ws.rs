...
async fn forward_events(
    mut events: tokio::sync::broadcast::Receiver<SquadEvent>,
    ...
    loop {
        ...
        let is_session_status = matches!(event, SquadEvent::SessionStatus { .. });
        let is_low_sev = matches!(
            &event,
            SquadEvent::FeedItem { item, .. } if is_low_severity_feed(item),
        );
