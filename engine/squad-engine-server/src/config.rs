...
/// `SQUAD_TUNNEL_URL` for local `wrangler dev`.
///
/// Must stay in sync with `mobile/src/lib/pairing.ts::DEFAULT_RELAY` and
/// with the production `routes` entry in `squad-relay/wrangler.toml`.
pub const DEFAULT_RELAY_URL: &str = "https://tunnel.getsquad.ai";
...
pub struct ServerConfig {
    ...
    pub bind: SocketAddr,
    /// Bearer token clients must send. Auto-generated if `SQUAD_ENGINE_TOKEN` unset.
    pub token: String,
    /// Houston home directory (`~/.squad` by default). Holds `engine.json`.
    pub home_dir: PathBuf,
    /// Houston docs directory (`~/Documents/Houston` by default).
    pub docs_dir: PathBuf,
    /// Product-layer system prompt supplied by the app via
    /// `SQUAD_APP_SYSTEM_PROMPT`. Engine prepends to every session when the
    ...
    /// `SQUAD_APP_ONBOARDING_PROMPT`. Appended on first-run sessions.
    pub app_onboarding_prompt: String,
    /// Houston relay base URL. Always populated — defaults to
    /// [`DEFAULT_RELAY_URL`], overridable via `SQUAD_TUNNEL_URL`.
...
impl ServerConfig {
    pub fn from_env() -> Self {
        let bind: SocketAddr = std::env::var("SQUAD_BIND")
            .ok()
        ...
        let token = std::env::var("SQUAD_ENGINE_TOKEN").unwrap_or_else(|_| gen_token());

        // Debug vs release paths:
        //   release → `~/.squad/` (home) + `~/Documents/Houston/` (docs,
        ...
        // Env vars still win.
        let home_dir = std::env::var("SQUAD_HOME")
            .map(PathBuf::from)
            .unwrap_or_else(|_| {
                dirs_home()
                    .map(|h| {
        ...
        let docs_dir = std::env::var("SQUAD_DOCS")
            .map(PathBuf::from)
            .unwrap_or_else(|_| {
                // Workspaces always live under `$SQUAD_HOME/workspaces/` so
                // everything Houston writes is rooted at one discoverable
                // location. Applies to both debug (`~/.dev-houston/`) and
                // release (`~/.squad/`). The Houston desktop app handles a
        ...
        let app_system_prompt = std::env::var("SQUAD_APP_SYSTEM_PROMPT").unwrap_or_default();
        let app_onboarding_prompt =
            std::env::var("SQUAD_APP_ONBOARDING_PROMPT").unwrap_or_default();

        let tunnel_url = std::env::var("SQUAD_TUNNEL_URL")
            .ok()
            .filter(|s| !s.is_empty())
