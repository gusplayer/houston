...
//!
//! Emits `SquadEvent::ComposioCliReady` on success so the frontend
...
const PREF_CLI_VERSION: &str = "composio_cli_squad_version";
...
pub async fn ensure_and_upgrade(sink: DynEventSink, db: Database) {
    if install::is_bundled() {
        ...
        sink.emit(SquadEvent::ComposioCliReady);
        return;
    }
    ...
    if !install::is_installed() {
        ...
        match install::install().await {
            ...
            Err(e) => {
                ...
                sink.emit(SquadEvent::ComposioCliFailed { message: e });
                return;
            }
        }
    }
