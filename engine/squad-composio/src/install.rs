...
//!    `squad_cli_bundle`. No install step — `install()` is a no-op
...
//!    with developers running `cargo run -p squad-engine-server`
...
/// production builds resolve via `squad_cli_bundle` instead.
...
pub fn cli_path() -> PathBuf {
    squad_cli_bundle::bundled_composio_binary().unwrap_or_else(standalone_cli_path)
}

/// Resolve the active install directory (the parent of `cli_path`,
/// containing the binary plus its sibling files).
pub fn install_dir() -> PathBuf {
    squad_cli_bundle::bundled_composio_dir().unwrap_or_else(standalone_install_dir)
}
...
pub fn is_bundled() -> bool {
    squad_cli_bundle::bundled_composio_binary().is_some()
}
...
pub async fn install() -> Result<PathBuf, String> {
    if let Some(p) = squad_cli_bundle::bundled_composio_binary() {
        tracing::info!(
...
pub async fn install() -> Result<PathBuf, String> {
    ...
    let result = tokio::time::timeout(
        ...
        tokio::task::spawn_blocking(move || {
            let tmp_out = std::env::temp_dir().join("squad-composio-install-stdout.log");
            let tmp_err = std::env::temp_dir().join("squad-composio-install-stderr.log");
...
/// rest of `squad-composio` doesn't read `$HOME` directly — that env
...
mod tests {
    ...
    /// that fake an `.app` layout in `squad-cli-bundle::tests`.
    #[test]
    fn cli_path_falls_back_to_standalone_in_dev() {
        ...
        // dev-workspace resolver added in squad-cli-bundle is gated on
