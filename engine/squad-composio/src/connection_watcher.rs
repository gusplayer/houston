...
//! `SquadEvent::ComposioConnectionAdded` exactly once when the
...
///
/// Emits `SquadEvent::ComposioConnectionAdded { toolkit }` on
...
async fn run_watch(slug: String, sink: DynEventSink) {
    ...
    loop {
        ...
        if connected.iter().any(|s| s == &slug) {
            ...
            sink.emit(SquadEvent::ComposioConnectionAdded {
                toolkit: slug.clone(),
            });
            break;
        }
