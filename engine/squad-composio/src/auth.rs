...
async fn run_callback_listener() -> Result<(), String> {
    ...
    send_response(
        ...
        "You can close this tab and return to Squad.",
    )
    .await;
...
async fn discover_oauth_metadata() -> Result<DiscoveryInfo, String> {
    ...
    let init_body = serde_json::json!({
        ...
        "params": {
            ...
            "capabilities": {},
            "clientInfo": { "name": "squad", "version": "0" }
        }
    });
...
const SQUAD_COMPOSIO_KEY: &str = "composio|houston";
...
/// `SQUAD_COMPOSIO_KEY` with the full `discoveryState` shape that Claude
...
fn update_keychain_token(
    ...
    let target_keys: Vec<String> = if existing_keys.is_empty() {
        mcp_oauth.insert(
            SQUAD_COMPOSIO_KEY.to_string(),
            serde_json::json!({}),
        );
        vec![SQUAD_COMPOSIO_KEY.to_string()]
    } else {
        existing_keys
    };
