//! squad-agent-files — generic file-level access to an agent's `.squad/` directory.
//!
//! Each data "type" lives in its own folder with a co-located JSON Schema:
//!   .squad/<type>/<type>.json
//!   .squad/<type>/<type>.schema.json
//!
...
/// Returns the first path component of `.squad/<type>/...` — e.g. "activity".
pub fn classify(rel: &str) -> Option<String> {
    let p = Path::new(rel);
    let mut it = p.components();
    // Expect first component to be ".squad"
    let first = it.next()?;
    let s = match first {
...
/// Seed the five embedded JSON Schemas under `.squad/<type>/<type>.schema.json`.
...
pub fn seed_schemas(agent_root: &Path) -> Result<()> {
    for (name, content) in schemas::ALL {
        let rel = format!(".squad/{name}/{name}.schema.json");
...
///   .squad/activity.json
///   .squad/routines.json
///   .squad/routine_runs.json
///   .squad/config.json
///   .squad/memory/learnings.md
///
/// New:
///   .squad/activity/activity.json
///   .squad/routines/routines.json
///   .squad/routine_runs/routine_runs.json
///   .squad/config/config.json
///   .squad/learnings/learnings.json
///
...
pub fn migrate_agent_data(agent_root: &Path) -> Result<()> {
    ...
    for name in ["activity", "routines", "routine_runs", "config"] {
        let old_rel = format!(".squad/{name}.json");
        let new_rel = format!(".squad/{name}/{name}.json");
    ...
    let learnings_md = agent_root.join(".squad/memory/learnings.md");
    let learnings_new = agent_root.join(".squad/learnings/learnings.json");
    if learnings_md.exists() && !learnings_new.exists() {
        ...
        write_file_atomic(agent_root, ".squad/learnings/learnings.json", &body)?;
    ...
    // `.squad/prompts/`. These were never user-editable through any UI —
    ...
    // `modes/` are untouched.
    for legacy in [
        ".squad/prompts/system.md",
        ".squad/prompts/self-improvement.md",
    ] {
...
mod tests {
    ...
    fn roundtrip_write_read() {
        ...
        write_file_atomic(dir.path(), ".squad/activity/activity.json", "[]").unwrap();
        let got = read_file(dir.path(), ".squad/activity/activity.json").unwrap();
        assert_eq!(got, "[]");
    }
    ...
    fn migrate_moves_legacy_files() {
        let dir = TempDir::new().unwrap();
        let legacy = dir.path().join(".squad/activity.json");
        fs::create_dir_all(legacy.parent().unwrap()).unwrap();
        fs::write(&legacy, "[{\"id\":\"a\"}]").unwrap();

        migrate_agent_data(dir.path()).unwrap();

        let new = dir.path().join(".squad/activity/activity.json");
        assert!(new.exists());
    ...
    fn migrate_removes_legacy_product_prompts() {
        let dir = TempDir::new().unwrap();
        let prompts = dir.path().join(".squad/prompts");
    ...
    fn migrate_learnings_md_to_json() {
        let dir = TempDir::new().unwrap();
        let md = dir.path().join(".squad/memory/learnings.md");
        fs::create_dir_all(md.parent().unwrap()).unwrap();
        fs::write(&md, "- first learning\n- second learning\n").unwrap();

        migrate_agent_data(dir.path()).unwrap();

        let json = fs::read_to_string(dir.path().join(".squad/learnings/learnings.json")).unwrap();
