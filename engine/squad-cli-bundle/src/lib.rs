...
//! `squad-claude-installer` downloads it on first launch using URLs +
...
//! builds (`pnpm tauri dev`, `cargo run -p squad-engine-server`)
...
//!   Contents/
//!     MacOS/
//!       squad-app                ← parent app binary
//!       squad-engine[-<triple>]  ← engine sidecar (current_exe here)
//!     Resources/
...
//!   squad-app.exe
//!   squad-engine.exe                ← current_exe here
//!   resources\
//!     bin\                            ← bundled_bin_dir() returns this
//!       ...
//! ```
...
/// squad-engine-core::git_bash). Returns `None` on macOS / Linux
...
//
// Reading the manifest lets callers (notably squad-claude-installer)
...
pub fn host_platform_key() -> &'static str {
    ...
    // Surface laptops returns "unknown" here, and squad-claude-installer
...
/// Resources/ tree like macOS). Both `squad-app.exe` and the
/// `squad-engine.exe` sidecar live at the install root, so the
...
/// release}/squad-engine` (i.e. `pnpm tauri dev` or `cargo run`), the
...
fn dev_workspace_bin_dir(exe: &Path) -> Option<PathBuf> {
    ...
    // `pnpm tauri dev` / `cargo run -p squad-engine-server` invocations.
...
mod tests {
    ...
    fn fake_app(root: &Path, bin_contents: &[&str]) -> (PathBuf, PathBuf) {
        ...
        let exe = macos.join("squad-engine");
    ...
    fn rejects_random_path() {
        let tmp = tempfile::tempdir().unwrap();
        let exe = tmp.path().join("random").join("squad-engine");
    ...
    fn detects_windows_sibling_layout() {
        ...
        let exe = exe_dir.join("squad-engine");
        fs::write(&exe, b"").unwrap();
        let bin = exe_dir.join("resources").join(BIN_SUBDIR);
        fs::create_dir_all(&bin).unwrap();
    ...
    fn detects_windows_msi_install_layout() {
        ...
        // install root next to squad-app.exe.
        let tmp = tempfile::tempdir().unwrap();
        let install = tmp.path().join("Houston");
        fs::create_dir_all(&install).unwrap();
        let exe = install.join("squad-engine.exe");
        fs::write(&exe, b"").unwrap();
