...
//!      `squad-cli-bundle`). The upstream install script chases
...
//!   4. **Progress events** — the installer emits `SquadEvent`s so
...
//!    `SquadEvent::ClaudeCliInstalling { progress_pct }` every ~250 ms.
...
use squad_db::db::Database;
use squad_ui_events::{DynEventSink, SquadEvent};
use sha2::{Digest, Sha256};
use std::path::PathBuf;
use tokio::io::AsyncWriteExt;

// Path-resolution functions live in `squad-terminal-manager` so the
...
// `provider/resolve.rs`).
pub use squad_terminal_manager::claude_install_path::{
...
pub async fn ensure_and_upgrade(sink: DynEventSink, db: Database) {
    ...
    let Some(manifest) = resolve_manifest() else {
        ...
        sink.emit(SquadEvent::ClaudeCliReady);
        return;
    };
    ...
    sink.emit(SquadEvent::ClaudeCliInstalling { progress_pct: 0 });

    let sink_for_progress = sink.clone();
    let result = install(&entry, move |pct| {
        sink_for_progress.emit(SquadEvent::ClaudeCliInstalling { progress_pct: pct });
    })
    .await;
    ...
    match result {
        ...
        Err(e) => {
            ...
            sink.emit(SquadEvent::ClaudeCliFailed { message: e });
        }
    }
}
...
/// running `cargo run -p squad-engine-server` against an unbundled
/// build still get auto-install.
fn resolve_manifest() -> Option<squad_cli_bundle::CliDepsManifest> {
    if let Some(m) = squad_cli_bundle::load_bundled_manifest() {
        return Some(m);
    }
    ...
    loop {
        ...
        if candidate.is_file() {
            return squad_cli_bundle::CliDepsManifest::load(&candidate).ok();
        }
        match here.parent() {
            Some(p) => here = p,
            None => return None,
...
pub async fn install(
    entry: &squad_cli_bundle::CliEntry,
...
pub async fn install_to(
    ...
    let platform = squad_cli_bundle::host_platform_key();
    let url = entry
        .url_for(platform)
...
mod tests {
    ...
    fn entry_for(server_uri: &str, payload: &[u8]) -> squad_cli_bundle::CliEntry {
        ...
        let manifest = serde_json::json!({
            "claude-code": {
                ...
                "license": "PROPRIETARY",
                "urls": {
                    squad_cli_bundle::host_platform_key(): format!("{server_uri}/claude")
                },
                "checksums": {
                    squad_cli_bundle::host_platform_key(): actual
                }
            }
        });
        ...
        let m = squad_cli_bundle::CliDepsManifest::load(tmp.path()).unwrap();
    ...
    async fn install_rejects_checksum_mismatch_and_cleans_temp() {
        ...
        let manifest = serde_json::json!({
            "claude-code": {
                ...
                "binary_name": "claude",
                "urls": {
                    squad_cli_bundle::host_platform_key(): format!("{}/claude", server.uri())
                },
                "checksums": {
                    squad_cli_bundle::host_platform_key():
        ...
        let entry = squad_cli_bundle::CliDepsManifest::load(tmp.path())
            .unwrap()
            .entry("claude-code")
            .unwrap();
