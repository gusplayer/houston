...
//! `SquadEvent` variants when files change. This catches writes made
...
/// Map a changed file path to the appropriate `SquadEvent`.
fn classify_change(agent_path: &str, relative: &Path) -> Option<SquadEvent> {
    let rel_str = relative.to_string_lossy();

    // .agents/skills/ (skill.sh convention — source of truth for skills)
    if rel_str.starts_with(".agents/skills") {
        return Some(SquadEvent::SkillsChanged {
    ...
    // .squad/ internal files — per-type folder layout: .squad/<type>/<type>.json
    if rel_str.starts_with(".squad/") {
        let inner = &rel_str[".squad/".len()..];
        ...
        return match first {
            "activity" => Some(SquadEvent::ActivityChanged { agent_path }),
            "routines" => Some(SquadEvent::RoutinesChanged { agent_path }),
            "routine_runs" => Some(SquadEvent::RoutineRunsChanged { agent_path }),
            "config" => Some(SquadEvent::ConfigChanged { agent_path }),
            "learnings" => Some(SquadEvent::LearningsChanged { agent_path }),
            _ => None,
        };
    }
    ...
    // Non-.squad files (user documents, etc.)
    Some(SquadEvent::FilesChanged {
        agent_path: agent_path.to_string(),
    })
}

/// Start watching an agent directory for file changes. Emits `SquadEvent`
    ...
    let mut debouncer = new_debouncer(
        ...
        move |events: Result<Vec<notify_debouncer_mini::DebouncedEvent>, notify::Error>| {
            ...
            for event in events {
                ...
                if let Some(squad_event) = classify_change(&ws_path, relative) {
                    // Use the event type name as dedup key
                    let key = format!("{:?}", std::mem::discriminant(&squad_event));
                    if emitted.insert(key.clone()) {
                        tracing::debug!("[watcher] emit: {key} for {}", relative.display());
                        sink.emit(squad_event);
                    }
                }
            }
        },
