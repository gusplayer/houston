...
/// `<workspace-agent>/.squad/bundled-package.json`. Reading it lets
/// the next sync pass know which migration steps still need applying.
const BUNDLED_PACKAGE_MARKER: &str = ".squad/bundled-package.json";
...
/// we use in `squad.json` / `catalog.json`. Treats missing trailing
...
fn bundled_store_root() -> Option<PathBuf> {
    if let Ok(dir) = std::env::var("SQUAD_STORE_DIR") {
...
pub(super) fn install_bundled_agent(
    ...
    if !source_dir.join("squad.json").exists() {
...
pub(super) fn sync_bundled_agent_instances(
    ...
    for workspace in workspaces::read_all(docs_dir)? {
        ...
        for entry in fs::read_dir(&workspace_dir)?.flatten() {
            ...
            let meta_path = folder.join(".squad").join("agent.json");
            if !meta_path.exists() {
                continue;
            }
...
fn clear_seeded_intro_activity(agent_dir: &Path) -> CoreResult<bool> {
    ...
    for relative in [".squad/activity.json", ".squad/activity/activity.json"] {
...
fn sync_existing_skill_metadata(source_dir: &Path, target_dir: &Path) -> CoreResult<bool> {
    ...
    let (mut source_summary, _) = match squad_skills::format::parse_file(&source_md) {
        Ok(parsed) => parsed,
        Err(e) => {
            tracing::warn!(
    ...
    let (target_summary, target_body) = match squad_skills::format::parse_content(&target_raw) {
    ...
    let updated = squad_skills::format::serialize(&source_summary, &target_body);
...
mod tests {
  ...
  "agents": [{{
    ...
    fn bundled_catalog_reads_from_store_dir() {
        ...
        std::env::set_var("SQUAD_STORE_DIR", store.path());

        let catalog = bundled_catalog().unwrap().unwrap();

        std::env::remove_var("SQUAD_STORE_DIR");
        assert_eq!(catalog.len(), 1);
        assert_eq!(catalog[0].repo, "squad-store/demo");
...
"#,
        ...
        let (summary, body) = squad_skills::format::parse_content(&updated).unwrap();
    ...
    fn sync_bundled_agent_instances_removes_seeded_intro_activity_only() {
        ...
        let legacy_activity = agent_dir.join(".squad/activity.json");
        let nested_activity = agent_dir.join(".squad/activity/activity.json");
    ...
    fn bundled_store_skills_parse_without_forms() {
        ...
        for file in &files {
            let (summary, _) = squad_skills::format::parse_file(file)
    ...
    fn bundled_store_agents_do_not_seed_activity_cards() {
        ...
        for entry in entries.flatten() {
            let manifest_path = entry.path().join("squad.json");
            ...
            assert!(
                !seeds.contains_key(".squad/activity.json"),
                "{} must not seed legacy activity cards",
                manifest_path.display()
            );
            assert!(
                !seeds.contains_key(".squad/activity/activity.json"),
  ...
  {"from": "0.1.4", "to": "0.2.0", "renames": {"old-slug": "new-slug"}}
        ...
        // rename step.
        let marker_body =
            fs::read_to_string(docs.path().join("Acme/Ops/.squad/bundled-package.json"))
