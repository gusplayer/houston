...
//!   `squad-agents-conversations`, streaming updates to the engine's event
...
use squad_agents_conversations::session_id_tracker::SessionIdTracker;
use squad_agents_conversations::session_pids::SessionPidMap;
use squad_agents_conversations::session_runner::{self, PersistOptions};
use squad_db::Database;
use squad_terminal_manager::{FeedItem, Provider};
...
async fn run_start(
    ...
    //   embedding app (Houston desktop) passed in via `SQUAD_APP_SYSTEM_PROMPT`.
    ...
    match crate::agents::activity::set_status_by_session_key(&agent_dir, &session_key, "running") {
        Ok(Some(_)) => {
            events.emit(SquadEvent::ActivityChanged {
            ...
            match file_changes::snapshot(&working_dir_for_end) {
                Ok(after) => {
                    ...
                    if !changes.is_empty() {
                        ...
                        events_for_end.emit(SquadEvent::FeedItem {
                            agent_path: agent_path_for_end.clone(),
                            session_key: session_key_for_end.clone(),
                            item,
                        });
                        events_for_end.emit(SquadEvent::FilesChanged {
                            agent_path: agent_path_for_end.clone(),
                        });
    ...
    match crate::agents::activity::set_status_by_session_key(
        ...
        Ok(Some(_)) => {
            ...
            events_for_end.emit(SquadEvent::ActivityChanged {
...
pub async fn cancel(
    ...
    events.emit(SquadEvent::FeedItem {
...
pub async fn start_onboarding(
    ...
    let anthropic_api_key_override = if resolved.uses_squad_credits {
        provider::squad_credits_key()
    } else {
        None
    };
...
mod tests {
    use super::*;
    use squad_db::Database;
    use squad_ui_events::NoopEventSink;
    use std::sync::Arc;
