...
impl From<SkillError> for CoreError {
    fn from(err: SkillError) -> Self {
        use squad_engine_protocol::ErrorCode;
        match err {
...
fn emit_skills_changed(events: &DynEventSink, workspace_path: &str) {
    events.emit(SquadEvent::SkillsChanged {
...
pub fn list(workspace_path: &str) -> CoreResult<Vec<SkillSummaryResponse>> {
    ...
    let summaries = squad_skills::list_skills(&dir)?;
    for s in &summaries {
        ...
        .map(|s| SkillSummaryResponse {
            ...
            inputs: s
                ...
                .map(|i| SkillInputResponse {
                    ...
                    kind: match i.kind {
                        squad_skills::SkillInputKind::Text => "text".into(),
                        squad_skills::SkillInputKind::Textarea => "textarea".into(),
                        squad_skills::SkillInputKind::Select => "select".into(),
                    },
                    required: i.required,
                    default: i.default,
                    options: i.options,
...
pub fn load(workspace_path: &str, name: &str) -> CoreResult<SkillDetailResponse> {
    let dir = skills_dir(workspace_path);
    let skill = squad_skills::load_skill(&dir, name)?;
...
pub fn delete(events: &DynEventSink, workspace_path: &str, name: &str) -> CoreResult<()> {
    let dir = skills_dir(workspace_path);
    squad_skills::delete_skill(&dir, name)?;
...
pub fn save(events: &DynEventSink, name: &str, req: SaveSkillRequest) -> CoreResult<()> {
    let dir = skills_dir(&req.workspace_path);
    squad_skills::edit_skill(&dir, name, &req.content)?;
    emit_skills_changed(events, &req.workspace_path);
    Ok(())
}

pub async fn list_from_repo(source: &str) -> CoreResult<Vec<RepoSkill>> {
    squad_skills::remote::list_skills_from_repo(source)
        .await
        .map_err(Into::into)
}
...
pub async fn install_from_repo(
    ...
    let repo_skills: Vec<RepoSkill> = req
        ...
        .collect();
    let names = squad_skills::remote::install_from_repo(&dir, &req.source, &repo_skills).await?;
    for n in &names {
...
pub async fn search_community(query: &str) -> CoreResult<Vec<CommunitySkill>> {
    squad_skills::remote::search_skills(query)
        .await
        .map_err(Into::into)
}
...
pub async fn popular_community() -> CoreResult<Vec<CommunitySkill>> {
    squad_skills::remote::fetch_popular_skills()
        .await
        .map_err(Into::into)
}
...
pub async fn install_community(
    ...
    let dir = skills_dir(&req.workspace_path);
    let name = squad_skills::remote::install_skill(&dir, &req.source, &req.skill_id).await?;
...
mod tests {
    use super::*;
    use squad_ui_events::{BroadcastEventSink, DynEventSink};
    use std::sync::Arc;
    ...
    async fn create_then_list_and_load() {
        ...
        matches!(ev, SquadEvent::SkillsChanged { .. });
    ...
    fn create_duplicate_conflicts() {
        ...
        .unwrap_err();
        assert_eq!(err.code(), squad_engine_protocol::ErrorCode::Conflict);
    ...
    fn load_missing_is_not_found() {
        ...
        assert_eq!(err.code(), squad_engine_protocol::ErrorCode::NotFound);
