//! Typed read/write operations for `.squad/` agent files.
//!
//! Every Houston agent project stores agent-visible data in a `.squad/` folder
...
//!
//! Relocated from `app/squad-tauri/src/agent_store/` as part of the engine
...
//! (`squad_engine_core::conversations`) and is already REST-exposed under
...
/// File-backed store for `.squad/` agent data.
///
...
impl AgentStore {
    ...
    pub fn ensure_squad_dir(&self) -> CoreResult<()> {
        store::ensure_squad_dir(&self.root)
    }
