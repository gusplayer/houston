...
//!    operate on `.squad/<type>/<type>.json` and friends, with path-traversal
//!    safety enforced by `squad-agent-files`. Writes return the matching
//!    `SquadEvent` so the caller (REST handler or Tauri command) can fan it
...
//!    agent's CLAUDE.md (not under `.squad/`).
//!
//! Relocated from `app/squad-tauri/src/agent_files.rs` and the FS-ops chunk
//! of `app/squad-tauri/src/agent_commands.rs` as part of the engine-standalone
...
/// `SquadEvent` that the caller should emit (or `None` for paths outside
/// the typed `.squad/` layout).
pub fn write_agent_file(
    agent_root: &Path,
    agent_path: &str,
    rel_path: &str,
    content: &str,
) -> CoreResult<Option<SquadEvent>> {
    files::write_file_atomic(agent_root, rel_path, content).map_err(file_err)?;
    Ok(event_for_write(agent_path, rel_path))
}

/// Seed the embedded JSON Schemas into `.squad/<type>/<type>.schema.json`.
pub fn seed_agent_schemas(agent_root: &Path) -> CoreResult<()> {
    files::seed_schemas(agent_root).map_err(file_err)
}

/// Idempotent migration from the legacy flat `.squad/` layout.
pub fn migrate_agent_files(agent_root: &Path) -> CoreResult<()> {
    files::migrate_agent_data(agent_root).map_err(file_err)
}

fn event_for_write(agent_path: &str, rel_path: &str) -> Option<SquadEvent> {
    if rel_path == "CLAUDE.md" || rel_path.starts_with(".squad/prompts/") {
        return Some(SquadEvent::ContextChanged {
    ...
    Some(match kind.as_str() {
        "activity" => SquadEvent::ActivityChanged { agent_path },
        "routines" => SquadEvent::RoutinesChanged { agent_path },
...
mod tests {
    ...
    fn read_missing_returns_empty() {
        let d = tmp();
        let s = read_agent_file(d.path(), ".squad/activity/activity.json").unwrap();
        assert_eq!(s, "");
    }
    ...
    fn write_and_read_round_trip() {
        ...
        .unwrap();
        assert!(matches!(ev, Some(SquadEvent::ActivityChanged { .. })));
        let s = read_agent_file(d.path(), ".squad/activity/activity.json").unwrap();
        assert_eq!(s, "[]");
    }
    ...
    fn write_claude_md_emits_context_changed() {
        ...
        assert!(matches!(ev, Some(SquadEvent::ContextChanged { .. })));
    }
    ...
    fn list_project_files_skips_dotdirs_and_unknown_ext() {
        ...
        std::fs::create_dir_all(d.path().join(".squad/activity")).unwrap();
        std::fs::write(d.path().join(".squad/activity/activity.json"), "[]").unwrap();
    ...
    fn list_project_files_includes_md() {
        ...
        let names: Vec<&str> = files.iter().map(|f| f.name.as_str()).collect();
        assert!(
            names.contains(&"morning_brief.md"),
            ".md files should appear in the Files tab"
        );
        assert!(
            !names.contains(&"data.json"),
            ".json files should remain hidden"
        );
    }
