//! Conversation listing — derived view over `.squad/activity/activity.json`.
//!
//! Every conversation is an activity with a UUID-scoped session key
//! (`activity-{id}`). This module reads the JSON file directly via
//! `squad_agent_files` so it does not depend on the full agent_store
...
fn read_activities(root: &Path) -> CoreResult<Vec<ActivityRow>> {
    let contents = files::read_file(root, ".squad/activity/activity.json")
...
mod tests {
    ...
    fn seed(dir: &Path, activities: serde_json::Value) {
        let rel = dir.join(".squad").join("activity");
