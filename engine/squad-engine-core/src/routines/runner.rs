...
use crate::routines::{self, ensure_squad_dir};
...
/// real impl on top of `squad-agents-conversations::session_runner`; tests
...
pub async fn run_routine(
    ...
    ensure_squad_dir(&working_dir)?;
    let run = routine_runs::create(&working_dir, routine_id)?;
    events.emit(SquadEvent::RoutineRunsChanged {
    ...
    if is_silent {
        ...
        events.emit(SquadEvent::CompletionToast {
...
/// `squad_tauri::paths::expand_tilde` so the runner stays in the engine tree.
