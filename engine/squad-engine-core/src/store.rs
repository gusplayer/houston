...
fn store_api() -> String {
    std::env::var("SQUAD_STORE_API").unwrap_or_else(|_| STORE_API_DEFAULT.to_string())
}
...
pub async fn install_agent(agents_dir: &Path, req: InstallAgent) -> CoreResult<()> {
    if let Some(agent_id) = req.repo.strip_prefix("squad-store/") {
...
pub async fn install_agent_from_github(agents_dir: &Path, github_url: &str) -> CoreResult<String> {
    ...
    let config_bytes: Vec<u8> = fetch_github_raw(&owner, &repo, "squad.json")
        .await?
        .ok_or_else(|| CoreError::NotFound(format!("no squad.json in {owner}/{repo}")))?;
...
pub async fn check_agent_updates(agents_dir: &Path) -> CoreResult<Vec<String>> {
    ...
    for entry in entries.flatten() {
        ...
        if source["source"].as_str() == Some("squad-store") {
        ...
        let local_config = fs::read_to_string(path.join("squad.json")).unwrap_or_default();
        let remote_config = match fetch_github_raw(owner, repo_name, "squad.json").await {
        ...
        if config_changed {
            let _ = fs::write(path.join("squad.json"), &remote_config);
        }
...
pub async fn install_workspace_from_github(
    ...
    for agent_folder in &template.agents {
        ...
        let config_path = format!("{prefix}/squad.json");

        let config_bytes = match fetch_github_raw(&owner, &repo, &config_path).await {
            Ok(Some(bytes)) => bytes,
            _ => {
                tracing::warn!("[workspace-import] skipping {agent_folder}: no squad.json");
                continue;
            }
        };
        ...
        fs::write(def_dir.join("squad.json"), &config_bytes)?;
        ...
        fs::create_dir_all(agent_folder_path.join(".squad"))?;
        ...
        fs::write(
            agent_folder_path.join(".squad/agent.json"),
        ...
        let squad = agent_folder_path.join(".squad");
        if !squad.join("activity.json").exists() {
            let _ = fs::write(squad.join("activity.json"), "[]");
        }
        if !squad.join("config.json").exists() {
            let _ = fs::write(squad.join("config.json"), "{}");
        }

        let _ = squad_agent_files::migrate_agent_data(&agent_folder_path);
...
/// Default agents dir: `$HOME/.squad/agents`. Override via `SQUAD_AGENTS_DIR`.
pub fn default_agents_dir() -> PathBuf {
    if let Ok(p) = std::env::var("SQUAD_AGENTS_DIR") {
...
mod tests {
    ...
    fn parse_shorthand() {
        assert_eq!(
            parse_github_ref("gethouston/squad-store").unwrap(),
            ("gethouston".into(), "squad-store".into())
        );
    }
    ...
    fn parse_full_url() {
        assert_eq!(
            parse_github_ref("https://github.com/gethouston/squad-store").unwrap(),
            ("gethouston".into(), "squad-store".into())
        );
        assert_eq!(
            parse_github_ref("https://github.com/gethouston/squad-store/").unwrap(),
        ...
        assert_eq!(
            parse_github_ref("https://github.com/gethouston/squad-store/tree/main").unwrap(),
            ("gethouston".into(), "squad-store".into())
        );
    }
    ...
    fn uninstall_removes_dir() {
        ...
        fs::write(a.join("squad.json"), "{}").unwrap();
        assert!(a.exists());
