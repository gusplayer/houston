...
//!
//! Reads installed agent manifests from `<home>/agents/*/squad.json`.
...
///
/// Each subdirectory with a valid `squad.json` contributes one entry.
...
pub fn list_installed(home: &Path) -> CoreResult<Vec<InstalledConfig>> {
    ...
    for entry in fs::read_dir(&dir)?.flatten() {
        ...
        let config_path = path.join("squad.json");
...
mod tests {
    ...
    fn finds_valid_configs_skips_broken() {
        ...
        fs::write(broken.join("squad.json"), "{not json").unwrap();
