...
//!
//! Current IDs are provider-scoped under `.squad/sessions/{provider}/`.
//! The legacy flat `.squad/sessions/{session_key}.sid` path is still read as a
...
fn sessions_dir(agent_dir: &Path) -> PathBuf {
    agent_dir.join(".squad").join("sessions")
}
...
impl SessionIdTracker {
    ...
    /// `.squad/sessions/{provider}/{session_key}.sid` is stored.
...
mod tests {
    ...
    fn provider_paths_are_distinct() {
        ...
        assert!(anthropic.ends_with(".squad/sessions/anthropic/chat.sid"));
        assert!(openai.ends_with(".squad/sessions/openai/chat.sid"));
    }
