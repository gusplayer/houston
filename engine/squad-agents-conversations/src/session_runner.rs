...
pub struct PersistOptions {
    ...
    pub claude_session_id: Option<String>,
    /// Workspace UUID this session belongs to. When set, `TokenUsage` items
    /// upsert into `session_usage` so the workspace dashboard can roll up
    /// spend by agent. `None` skips usage tracking entirely.
        ...
        while let Some(update) = rx.recv().await {
            match update {
                            ...
                            AuthFeedAction::VerifyProviderStatus => {
                                ...
                                if auth_state != ProviderAuthState::Unauthenticated {
                                    ...
                                    if !sent_auth_required {
                                        ...
                                        sink.emit(SquadEvent::AuthRequired {
                ...
                SessionUpdate::Status(ref status) => {
                    ...
                    sink.emit(SquadEvent::SessionStatus {
                        agent_path: agent_path_for_events.clone(),
                        session_key: key.clone(),
                        status: status_str,
                        error: err,
...
fn serialize_for_persist(item: &FeedItem) -> Option<(String, String)> {
    match item {
        ...
        FeedItem::TokenUsage {
            ...
            Some(("token_usage".into(), data.to_string()))
        }
...
mod tests {
    ...
    fn serializes_tool_runtime_error_for_history() {
        let item = FeedItem::ToolRuntimeError {
            kind: squad_terminal_manager::ToolRuntimeErrorKind::LocalTool,
