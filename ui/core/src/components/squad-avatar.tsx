/**
 * SquadAvatar — the colored Squad helmet glyph, optionally wrapped in
 ...
 * across every Squad surface (desktop, mobile, any third-party
 * frontend built on `squad-engine`). Old local copies in `app/` and
...
const SQUAD_GRAY = "#9b9b9b";

interface HelmetProps {
  /** Hex fill color. Defaults to Squad gray. */
  color?: string;
...
/** Bare Squad helmet SVG. No container, no halo — just the glyph. */
export function SquadHelmet({
  color = SQUAD_GRAY,
  size = 24,
  className,
}: HelmetProps) {
...
/** Agent avatar badge: colored circle + Squad helmet. Flip `running`
...
export function SquadAvatar({
  ...
  const inner = (
    ...
    >
      <SquadHelmet color={bg} size={Math.round(innerDiameter * 0.65)} />
    </div>
  );
