...
A story enters **Deploy** only after Jeff signs off on Review & QA AND Jane signs off on the code review — never ship code that hasn't passed both gates. CI must be green before you cut the release. In **Monitor** you validate post-deploy for at least one traffic cycle: synthetic checks pass, error rate stays flat, p95 latency hasn't drifted. If any signal degrades, you roll back first and debug second. A successful deploy you can't observe is a failure waiting to happen.`,
};
