...
export function HoustonCreditsBanner() {
  const { t } = useTranslation("providers");
  const [topUpOpen, setTopUpOpen] = useState(false);
  const balance = useHoustonCreditsStore((s) => s.balance);
  const workspace = useWorkspaceStore((s) => s.current);

  if (workspace?.provider !== "houston-credits") return null;
...
export function HoustonCreditsFooterPill({ className }: { className?: string }) {
  const { t } = useTranslation("providers");
  const [topUpOpen, setTopUpOpen] = useState(false);
  const balance = useHoustonCreditsStore((s) => s.balance);
  const workspace = useWorkspaceStore((s) => s.current);

  if (workspace?.provider !== "houston-credits" || balance === null) return null;
